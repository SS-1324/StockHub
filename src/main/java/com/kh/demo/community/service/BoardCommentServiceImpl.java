...
public class BoardCommentServiceImpl implements BoardCommentService {
    ...
    public List<BoardCommentDto> getList(Long boardId, String loginMemberId) {
        ...
        for (BoardCommentDto comment : comments) {
            if (Boolean.TRUE.equals(comment.getDeleted())) {
                comment.setHighlightedContent("삭제된 댓글입니다.");
    ...
    public void delete(Long boardId, Long commentId, String loginMemberId) {
        ...
        // 답글은 유지해야 하므로 진짜 DELETE 대신 is_deleted 플래그만 세운다.
        int updated = boardCommentMapper.softDeleteById(boardId, commentId, loginMemberId);
        if (updated == 0) {
