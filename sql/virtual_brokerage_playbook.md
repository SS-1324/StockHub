# 가상 증권사 활동 밀키트 (Raw SQL Playbook)

이 문서는 "가상 증권사"(우리 사이트 바깥에 있다고 가정하지만, 실제로는 우리 백엔드 DB에 구현되어 있는 존재)가
매수/매도/입금/출금/상품가입/상품환매 같은 일을 처리했을 때, 그 결과가 우리 DB에 어떻게 반영되는지를
**Java 서비스 코드와 완전히 동일한 계산식**으로 재현한 raw SQL 모음입니다.

각 블록은 그대로 복사해서 앞부분의 `SET @변수 = 값;` 줄만 채운 뒤, 블록 전체를 한 번에 실행하면 됩니다.
mysql 세션 변수(`@변수`)를 이용해서 수수료·가중평균 단가 같은 계산을 SQL이 대신 해주기 때문에,
사람이 직접 암산해서 넣을 값은 계좌ID·종목/상품·수량 정도뿐입니다.

모든 블록은 `START TRANSACTION;` ~ `COMMIT;`으로 감싸져 있습니다. 잔고나 보유수량이 부족해서
중간 어느 문장에서 에러가 나더라도(예: `balance`는 `unsigned`라 음수가 되면 자동으로 에러가 남),
**그 앞에서 이미 실행된 문장까지 전부 자동으로 취소**됩니다 — "보유내역만 늘고 잔고는 안 줄어드는" 것 같은
반쪽짜리 상태가 절대 안 남는다는 뜻이에요. 값만 고쳐서 처음부터 다시 실행하면 됩니다.

> 실행 후에는 항상 대시보드(`/member/dashboard`)를 새로고침해서 반영을 확인하세요.
> 총자산/총손익/목표 진행률은 전부 라이브로 다시 계산되기 때문에 서버 재시작이 필요 없습니다
> (단, `stock.current_price`처럼 여러 계좌가 공유하는 값을 바꾸면 그 종목을 보유한 다른 계좌에도 영향을 줍니다).
...
(그 외 종목은 `current_price = 0`으로만 등록돼 있어 거래 시뮬레이션엔 안 맞습니다. 최신값은
`SELECT stock_code, stock_name, current_price FROM stock WHERE current_price > 0;`로 확인하세요.)
...
이 종목을 보유한 **모든 계좌**의 평가금액에 영향을 줍니다(개별 계좌 조작이 아님에 주의).
...
**바뀌는 테이블**: `account`(신규 행). 아직 아무 회원과도 연동되지 않은 상태로 생성되며,
대시보드의 "계좌 연동하기"에서 이 계좌번호+예금주명으로 연동해야 실제 회원 것이 됩니다.
...
2026-08-11에 위 1~6번 블록 전부를 spare 계좌(연동 안 된 테스트 계좌)로 실제 실행해서 검증했습니다:
- 매수 → 매도(전량, holding 삭제 확인) → 상품가입 → 상품환매(부분, 가중평균/잔여좌수 확인) → 출금까지
  순서대로 실행한 뒤 잔고가 원 단위까지 정확히 맞아떨어지는 것 확인.
- 처음 작성했을 땐 세션 변수(`@stock_code`)와 테이블 컬럼의 collation이 달라서 에러가 나는 문제,
  그리고 트랜잭션으로 안 감쌌을 때 뒷 문장이 실패해도 앞 문장(INSERT)은 이미 커밋되어버려
  보유내역만 남고 잔고는 안 줄어드는 반쪽짜리 상태가 되는 문제를 발견해서 둘 다 고쳤습니다.

## 참고: 왜 매번 `account.balance`도 같이 업데이트하나요?

`account.balance`는 대시보드가 매 요청마다 라이브로 읽는 값이라 즉시 반영됩니다. 반면
`account.profit_amount`/`return_rate`/`holding_stock_quantity`는 데모 데이터 생성기만 갱신하는
저장값이라(2026-08-11에 이 불일치를 실제로 발견하고 고쳤습니다 - `MyStockServiceImpl` 참고),
이 밀키트들에서는 아예 건드리지 않습니다 — 대신 `holding`/`product_holding`을 살아있는 값으로
직접 갱신하고, 대시보드는 거기서 매번 다시 계산합니다.
